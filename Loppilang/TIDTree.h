...
public:
	...
	Node(NodeType type, Node* parent, std::string name) : _type(type), _parent(parent), _name(name) {}
...
private:
	...
	std::string find_func(Node* cur, std::string& name, std::vector<std::string>& param_types, const std::vector<std::string>& path, int position) const;
	...
	void check_scope_path(Node* cur, const std::string& name, const std::vector<std::string>& path, int position) const;
