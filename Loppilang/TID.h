...
public:
	...
	void push_func(std::string name
		, std::string return_type
		, std::vector<std::pair<std::string, std::string>> params)
	{ _funcs.insert(name, return_type, params); }

	void push_type(std::string name, std::map<std::string, std::string> fields) { _types.insert(name, fields); }
	...
	{
		return _funcs.get_return_type(name);
	}
	std::map<std::string, std::string>& find_type(std::string name) { return _types.get_fields(name); }
};
