...
std::string TypeOpStack::max_byte(const std::string& l, const std::string r) const
{
...
std::string TypeOpStack::check_uno(std::string s, std::string op) const
{
    ...
    if (_logic_uno_ops.count(op))
        return "byte";

    return s;
}

std::string TypeOpStack::check_assignment(std::string s1, std::string s2, std::string op) const
{
...
bool TypeOpStack::is_op(std::string s) const
{
    ...
    if (_comp_ops.count(s) ||
        _logic_bin_ops.count(s) ||
        _logic_uno_ops.count(s) ||
        _bin_ops.count(s) ||
        _uno_ops.count(s) ||
        _assign_ops.count(s))
        return true;
...
std::string TypeOpStack::check_bin()
{
    ...
    std::string s1, s2, op;
    s2 = _stack.back(); _stack.pop_back();
