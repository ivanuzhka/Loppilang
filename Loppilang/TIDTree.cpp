...
std::pair<std::string, std::vector<std::string>> TIDTree::path_parser(std::string raw_name) const
{
...
std::map<std::string, std::string>& TIDTree::find_type(Node* cur, std::string name, std::vector<std::string> path, int position) const
{
	if (position == (int)path.size())
	{
...
void TIDTree::check_scope_path(Node* cur, const std::string& name, const std::vector<std::string>& path, int position) const
{
	if (position == (int)path.size())
	{
...
{
	auto [aim, path] = path_parser(name);
